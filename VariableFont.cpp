...
//
// このファイルは AviUtl / ExEdit2 向けのテキストプラグイン本体です。
// 主な機能:
// - DirectWrite/Direct2D を用いたテキストレイアウトと描画
// - バリアブルフォント（可変軸）のサポート
// - 影・縁取り・切り抜き・疑似スタイル（擬似ボールド/擬似イタリック）の実装
//
// ここに追加するコメントは読みやすさ向上のための注釈であり、動作には影響しません。
//----------------------------------------------------------------------------------
#ifndef NOMINMAX
#define NOMINMAX
#endif
...
void *items[] = {
	&group_font, &fontFile, &fontFamilyInput, &fontSize, &fontColor, &bold, &italic, &group_font_end,

	&group_layout, &imageWidth, &imageHeight, &textAlign, &charSpacing, &lineSpacing, &group_layout_end,
...
FILTER_PLUGIN_TABLE filter_plugin_table = {
	...
	L"テキスト(VF)",												   // ラベルの初期値
	L"Variable Font Text 2.1.0 By 黒猫大福",						   // プラグインの情報
	items,															   // 設定項目の定義
...
HRESULT RenderText(ID2D1DeviceContext6 *d2dContext, IDWriteTextLayout *textLayout, float offsetX /*=0*/, float offsetY /*=0*/)
{
	...
	auto applyOptionalFauxItalicTransform = [&](D2D1_MATRIX_3X2_F &prevTransform, float pivotX, float pivotY)
	{
		d2dContext->GetTransform(&prevTransform);
		if (g_useFauxItalic)
		{
			const D2D1_MATRIX_3X2_F toPivot = D2D1::Matrix3x2F::Translation(-pivotX, -pivotY);
			const D2D1_MATRIX_3X2_F fromPivot = D2D1::Matrix3x2F::Translation(pivotX, pivotY);
			d2dContext->SetTransform(fromPivot * fauxItalicMatrix * toPivot * prevTransform);
		}
	};
	...
	if (shadowEnabled.value && shadowBlur.value > 0.0 && shadowBrush)
	{
		...
		if (SUCCEEDED(d2dContext->CreateCommandList(&shadowCommandList)))
		{
			...
			applyOptionalFauxItalicTransform(prevTransform, offsetX, offsetY);
	...
	// 描画開始: 描画コマンドのバッチを開始し、ターゲットを透明でクリアします。
	// これにより前フレームの残像を消し、レンダリング領域を初期化します。
	d2dContext->BeginDraw();
	d2dContext->Clear(D2D1::ColorF(0, 0, 0, 0));

	// メイン描画で擬似イタリック（せん断）を適用する場合は、現在の変換を保存し
	// せん断行列を乗算してから描画を行います。描画後は必ず元の変換に戻します。
	D2D1_MATRIX_3X2_F prevMainTransform;
	applyOptionalFauxItalicTransform(prevMainTransform, offsetX, offsetY);

	// 影描画処理
	// - シャドウはまずコマンドリスト上にテキストを描画し、
	//   そのイメージに対してシャドウ/ガウスぼかしエフェクトを適用して生成します。
	// - エフェクトが利用できない場合は、コマンドリスト自体をそのまま描画します。
	// - エフェクトがない場合（shadowImage がない）には、単純にオフセットして描画します。
	// - 影にも擬似ボールドや擬似イタリックの効果を反映させるため、
	//   前段で適用した transform を有効にしたまま描画を行います。
	...
	// 縁取り（アウトライン）描画の準備と注意点
	// - 可能な限り正確なアウトラインを描画するために、カスタムレンダラー
	//   `OutlineTextRenderer` を使用して GlyphRun からジオメトリを取得し描画します。
	// - `ID2D1StrokeStyle` のプロパティで線の端点や結合方法を制御します。
	//   - `startCap` / `endCap`: 線端の形状（丸め / 平坦）
	//   - `lineJoin`: 線の結合方法（丸め / ミーター）
	//   - `miterLimit`: ミーター結合時の突き出し抑制
	// - カスタムレンダラーで描画できない場合（例: 実装失敗やサポート外フォント等）は
	//   フォールバックとして多方向にオフセットした塗りつぶしで擬似的に縁取りを表現します。
	// - アウトライン描画後の塗り（本文）の有無は `clipping` フラグに依存します。
	// - 擬似ボールド/イタリックを用いている場合でもアウトライン描画時に
	//   transform を適切に維持することで見た目を整合させます。
	bool outlined = false;
...
bool func_proc_video(FILTER_PROC_VIDEO *video)
{
	...
	float rightPad = outlinePad + shadowRight + blurMargin + fauxBoldPad + 0.0f * 0.5f;
	...
	if (autoW || autoH)
	{
		...
		// drawOffsetX = drawOffsetX - leftPad - rightPad;
		// drawOffsetY = drawOffsetY - topPad - bottomPad;
	}
	else
	{
